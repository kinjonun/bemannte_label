...
class EXPConfig:
    ...
    ida_conf = dict(resize_dims=(896, 512), up_crop_ratio=0.25, rand_flip=False, rot_lim=False)
    INPUT_SHAPE = [int(ida_conf["resize_dims"][1] * (1 - ida_conf["up_crop_ratio"])), int(ida_conf["resize_dims"][0])]

    map_conf = dict(
        dataset_name="nuscenes",
        nusc_root="data/nuscenes",
        anno_root="data/nuscenes/customer/bemapnet_location",
    ...
    point_cloud_range = [-15.0, -30.0, -10.0, 15.0, 30.0, 10.0]     # LSS 用
    ...
    model_setup = dict(
        ...
        map_decoder=dict(
            ...
            net_kwargs=dict(
                key='im_bkb_features',           # !
                use_fb_bev=True,
                fpn_channels=2048,               # 128 * 5    2048 bkb最后yiceng
                in_channels=256,
                out_channels=256,
                feat_down_sample=32,
        ...
        post_processor=dict(
            ...
            net_kwargs=dict(
                ...
                criterion_conf=dict(
                    ...
                    loss_weight=dict(
                        sem_loss=0.5, depth_loss=1, pv_seg_loss=1,
...
class Exp(BaseExp):
    ...
    def _configure_train_dataloader(self):
        ...
        train_set = NuScenesMapDatasetDepth(
            ...
            aux_seg_cfg=self.exp_config.aux_seg_cfg,
    ...
    def training_step(self, batch):
        ...
        # pdb.set_trace()
        if self.exp_config.aux_seg_cfg['pv_seg']:
            outputs["outputs"]['im_nek_features'] = outputs['im_nek_features'][0]
        # pdb.set_trace()
