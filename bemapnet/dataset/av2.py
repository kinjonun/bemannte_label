...
class Argoverse2MapDataset(Dataset):
    def __init__(self, img_key_list, map_conf, ida_conf, bezier_conf, transforms, data_split="training", splits_txt=None):
        super().__init__()
        self.splits_txt = splits_txt
        ...
        if map_conf.get('geo_splits', False):
            self.split_mode = 'geo_train' if data_split == "training" else 'geo_val'
        if self.splits_txt is None:
            split_path = os.path.join(self.split_dir, 'av2_'f'{self.split_mode}_list.txt')
        else:
            split_path = os.path.join(self.split_dir, self.splits_txt)
    ...
    def __getitem__(self, idx: int):
        ...
        for im_path in input_dict['img_filename']:
            if 'argoverse2' not in im_path:
                im_path = im_path.replace('av2', 'argoverse2/sensor')
        ...
        img_metas = {'map_location': input_dict["map_location"], 'lidar2global': lidar2global, 'split': input_dict['split']}
        ...
        item = dict(
            images=images, targets=dict(masks=masks, points=ctr_points, labels=ins_labels), extrinsic=np.stack(extrinsic),
            intrinsic=np.stack(intrinsic), ida_mats=np.stack(ida_mats), extra_infos=dict(token=token,
            img_key_list=self.img_key_list, map_size=self.ego_size, do_flip=flip, img_metas=img_metas)
        )
