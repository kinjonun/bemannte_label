...
from bemapnet.models.backbone.bifpn import BiFPN, FPN
import pdb
...
class ResNetBackbone(nn.Module):
    def __init__(self, bkb_kwargs, fpn_kwarg=None, up_shape=None, ret_layers=1):
        ...
        self.fpn = None if fpn_kwarg is None else FPN(**fpn_kwarg)
    ...
    def forward(self, inputs):
        ...
        bkb_features = list(self.bkb(images)[-self.ret_layers:])  # ([6, 512, 48, 112],[6, 1024, 24, 56],[6,2048, 12,28]
        # pdb.set_trace()
        nek_features = self.fpn(bkb_features) if self.fpn is not None else None      # [6, 640, 21, 49]
