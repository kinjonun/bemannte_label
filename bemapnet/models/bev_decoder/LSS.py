...
class BaseTransform(BaseModule):
    ...
    def get_geometry(
        ...
        # points = torch.matmul(img2lidar.to(torch.float32), points.unsqueeze(-1).to(torch.float32)).squeeze(-1)
        # import pdb;pdb.set_trace()
        eps = 1e-5
    ...
    def bev_pool(self, geom_feats, x):
        ...
        # flatten indices
        geom_feats = ((geom_feats - (self.bx - self.dx / 2.0)) / self.dx).long()     # [1, 6, 68, 12, 28, 3]
        ...
        x = bev_pool(x, geom_feats, B, self.nx[2], self.nx[0], self.nx[1])      # [1, 256, 1, 200, 400]
    ...
    def forward(self, images, img_metas):
        ...
        geom = self.get_geometry_v1(fH, fW, rots, trans, intrins, post_rots, post_trans, lidar2ego_rots, lidar2ego_trans,
                                    img_metas)                 # 注意 geom 是在 lidar 坐标系  [1, 6, 68, 12, 28, 3]
        # pdb.set_trace()
        mlp_input = self.get_mlp_input(camera2ego, camera_intrinsics, post_rots, post_trans)
        x, depth = self.get_cam_feats(images, mlp_input)       # [1, 6, 68, 12, 28, 256],   [1, 6, 68, 12, 28]
        # pdb.set_trace()
        x = self.bev_pool(geom, x)                             # [1, 256, 200, 400]
        x = x.permute(0, 1, 3, 2).contiguous()          # [1, 256, 400, 200]
...
class LSSTransform(BaseTransform):
    ...
    def get_cam_feats(self, x, mlp_input):
        # pdb.set_trace()
        B, N, C, fH, fW = x.shape              # [1, 6, 256, 12, 28]
        x = x.view(B * N, C, fH, fW)
        x = self.depth_net(x, mlp_input)                                           # [6, 324, 12, 28]
        depth = x[:, : self.D].softmax(dim=1)                                      # [6, 68, 12, 28]
        x = depth.unsqueeze(1) * x[:, self.D: (self.D + self.C)].unsqueeze(2)      # [6, 256, 68, 12, 28]

        x = x.view(B, N, self.C, self.D, fH, fW)                                   # [1, 6, 256, 68, 12, 28]
        x = x.permute(0, 1, 3, 4, 5, 2)
        depth = depth.view(B, N, self.D, fH, fW)                                   # [1, 6, 68, 12, 28]
        return x, depth

    def forward(self, images, img_metas):
        bs, n, cc, h, w = images.shape                             # [1, 6, 600, 12, 28]
        # pdb.set_trace()
        images = self.input_proj(images.view(bs*n, cc, h, w))      # [6, 256, 12, 28]
        c_ = images.shape[1]                                       # 256
        images = images.view(bs, n, c_, h, w)
        x, depth = super().forward(images, img_metas)     # [1, 256, 400, 200]; [1, 6, 68, 12, 28]
        # pdb.set_trace()
