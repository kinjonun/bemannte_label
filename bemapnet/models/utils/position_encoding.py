...
class PositionEmbeddingIPM(nn.Module):
    ...
    def __init__(self,
        ...
        h, w_expand = num_pos
        self.current_shape = (h, w_expand // num_camera)   # (21, 49)
        self.input_shape = input_shape                     # (512, 896)

        self.num_pos_feats = num_pos_feats         # 512
        self.temperature = temperature           # 10000
        self.encoder = encoder
        self.sine_encoding = sine_encoding       # True
    ...
    def get_embedding(self, extrinsic, intrinsic, ida_mats):
        ...
        world_coords = F.normalize(world_coords, p=2, dim=-1)                 # 对张量进行归一化,使用欧几里得范数（L2 范数）进行归一化,在最后一个维度上进行归一化
