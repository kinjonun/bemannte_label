...
class SetCriterion(nn.Module):
    ...
    def __init__(self, criterion_conf, matcher, num_degree, no_object_coe=1.0):
        ...
        # self.aux_seg = criterion_conf.get('aux_seg_cfg', None)
        # if self.aux_seg is not None:
        #     self.loss_pv_seg = build_loss(self.aux_seg['loss_pv_seg'])
        #     self.embed_dims = 256
        #     if self.aux_seg['pv_seg']:
        #         self.pv_seg_head = nn.Sequential(
        #             nn.Conv2d(600, self.embed_dims, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, outputs, targets):
        ...
        # pdb.set_trace()
        if 'lidar_depth' in outputs:
            losses.update(self.get_depth_loss(outputs['lidar_depth'], outputs['pred_depth']))
        # if self.aux_seg['pv_seg']:
        #     outputs_pv_seg = self.pv_seg_head(outputs['im_nek_features'])    # [12, 1, 12, 28]
        #     img_size = outputs_pv_seg.shape[1:]
        #     outputs_pv_seg = outputs_pv_seg.view(2, 6, *img_size)
        #     # pdb.set_trace()
        #     gt_pv_seg = []
        #     for i in range(len(targets)):
        #         gt_pv_seg.append(targets[i]['gt_pv_semantic_mask'])
        #     # gt_pv_seg = torch.cat(gt_pv_seg, dim=0)            # [12, 1, 12, 28]
        #     gt_pv_seg = torch.stack(gt_pv_seg, dim=0)
        #     pv_seg_loss = self.loss_pv_seg(outputs_pv_seg, gt_pv_seg.float())
        #     losses.update({'pv_seg_loss': pv_seg_loss})
        # pdb.set_trace()
...
class PiecewiseBezierMapPostProcessor(nn.Module):
    ...
    def refactor_targets(self, targets):
        ...
        if 'gt_pv_semantic_mask' in targets:
            targets["gt_pv_semantic_mask"] = targets["gt_pv_semantic_mask"].cuda()
        ...
        for batch_id in range(batch_size):
            ...
            if 'gt_pv_semantic_mask' in targets:
                targets_refactored.append({
                    ...
                    "valid_masks": valid_masks, 'gt_pv_semantic_mask': targets['gt_pv_semantic_mask'][batch_id],
                })
            else:
                targets_refactored.append({
                    "sem_masks": sem_masks, "ins_masks": ins_masks, "obj_labels": ins_objects,
    ...
    def post_processing(self, outputs):
        ...
        for i in range(batch_size):
            ...
            for j in range(self.num_classes):
                ...
                # pdb.set_trace()
                curve_pts[:, :, 0] *= self.map_size[1]    # 200
                curve_pts[:, :, 1] *= self.map_size[0]    # 400
                for dt_curve, dt_score in zip(curve_pts, pred_scores[keep_ids]):   # 遍历检测出的 n 个instance
                    cv2.polylines(masks[j], [dt_curve.astype(np.int32)], False, color=instance_index, thickness=self.save_thickness)
            ...
            batch_results.append({'map': points, 'confidence_level': scores, 'pred_label': labels, 'curve_pts': curve_pts})
