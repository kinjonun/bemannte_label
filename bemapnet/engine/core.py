...
from bemapnet.engine.callbacks import TensorBoardMonitor, TextMonitor, ClipGrad, SetEpochInfo, SaveHRMap
...
class BaseCli:
    ...
    def __init__(self, Exp):
        ...
        # self.exp_instance = copy.deepcopy(Exp())
        # pdb.set_trace()
    ...
    # def log_exp_config(self, logger):
    #     config_str = "\n".join(
    #         [f"{key}: {value}" for key, value in self.exp_instance.exp_config.model_setup.items() if not key.startswith('__')])
    #     logger.info("EXPConfig settings:\n" + config_str + "\n" + '-' * 120)

    def _set_basic_log_message(self, logger):              # 输出最初几行(1-52行)的日志
