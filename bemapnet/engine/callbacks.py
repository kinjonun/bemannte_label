...
__all__ = ["Callback", "MasterOnlyCallback", "CheckPointSaver", "CheckPointLoader", "CheckPointC2Loader",
           ...
           "LearningRateMonitor", "TextMonitor", "TensorBoardMonitor", "SetEpochInfo", "SaveHRMap"]


class MasterOnlyCallback(Callback):     # 只有主进程才可以调用
    enabled_rank = [0]
...
class CheckPointSaver(MasterOnlyCallback):
    ...
    def after_epoch(self, trainer: Trainer, epoch: int, update_best_ckpt: bool = False):   # 不能任一进程都能保存
...
class SetEpochInfo(Callback):
    ...
    def before_epoch(self, trainer: Trainer, epoch: int):
        ...
        model.ins_decoder.set_epoch(epoch)


class SaveHRMap(Callback):
    def after_train(self, trainer: Trainer):
        model = trainer.model
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            model = model.module
    ...
    def after_eval(self, trainer: Trainer):
        model = trainer.model
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            model = model.module
        model.return_map()
