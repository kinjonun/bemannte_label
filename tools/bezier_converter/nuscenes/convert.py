...
class NuScenesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        imgs, trans, rots, intrins, cam_ego_pose_trans, cam_ego_pose_rots, lidar_filename, lidar_tran, lidar_rot, \
            lidar_ego_pose_tran, lidar_ego_pose_rot, lidar2global_r, lidar2global_t, lidar2cam_rts, lidar2img_rts = self.get_data_info(record)
    ...
    def get_data_info(self, record):
        ...
        lidar2global_r = Quaternion(lidar_ego_pose_rot).rotation_matrix @ Quaternion(lidar_rot).rotation_matrix
        lidar2global_t = lidar_ego_pose_tran + Quaternion(lidar_ego_pose_rot).rotation_matrix @ lidar_tran

        lidar2cam_rts, lidar2img_rts = [], []
        for cam in ['CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_BACK_LEFT', 'CAM_BACK', 'CAM_BACK_RIGHT']:
            ...
            cam2global_r = Quaternion(cam_ego_pose['rotation']).rotation_matrix @ Quaternion(sens['rotation']).rotation_matrix
            cam2global_t = cam_ego_pose['translation'] + Quaternion(cam_ego_pose['rotation']).rotation_matrix @ sens['translation']
            ...
            viewpad = np.eye(4)
            viewpad[:3, :3] = sens['camera_intrinsic']
            lidar2img_rt = (viewpad @ lidar2cam_rt)
            lidar2img_rts.append(lidar2img_rt)
            # pdb.set_trace()

        return imgs, trans, rots, intrins, cam_ego_pose_trans, cam_ego_pose_rots, lidar_filename, lidar_tran, lidar_rot, \
            lidar_ego_pose_tran, lidar_ego_pose_rot, lidar2global_r, lidar2global_t, lidar2cam_rts, lidar2img_rts
...
class NuScenesSemanticDataset(NuScenesDataset):
    ...
    def __getitem__(self, idx):
        ...
        # lidar2global_t:      [2.37609280e+03, 7.96460203e+02, 1.83787820e+00]
        # lidar_ego_pose_tran: [2377.076012626698, 796.5767858112583, 0.0]
        # cam_ego_pose_trans: [[2377.0921251008176, 796.5772370708174, 0.0],
        # pdb.set_trace()
        lidar2global_r_q = Quaternion(matrix=lidar2global_r)
        rotation = Quaternion(lidar_ego_pose_rot)
        patch_angle = quaternion_yaw(rotation) / np.pi * 180
        patch_angle_lidar = quaternion_yaw(lidar2global_r_q) / np.pi * 180
        vectors = self.vector_map.gen_vectorized_samples(location, lidar2global_t, lidar2global_r_q)   # 注意是lidar坐标系不是自车
        ...
        return imgs, np.stack(trans), np.stack(rots), np.stack(intrins), semantic_masks, instance_masks, vectors, \
            ...
            lidar_filename, np.stack(lidar_tran), np.stack(lidar_rot), np.stack(lidar_ego_pose_tran), \
            np.stack(lidar_ego_pose_rot), np.stack(lidar2global_r), np.stack(lidar2global_t), np.stack(lidar2cam_rts), \
            np.stack(lidar2img_rts), location
...
def main():
    ...
    parser.add_argument("--xbound", nargs=3, type=float, default=[-15.0, 15.0, 0.15])   # -30.0, 30.0, 0.15
    ...
    for version in args.version:
        ...
        for idx in tqdm(range(dataset.__len__())):
            ...
            np.savez_compressed(
                ...
                lidar_ego_pose_tran=item[14], lidar_ego_pose_rot=item[15], lidar2global_r=item[16],
                lidar2global_t=item[17], lidar2cam_rts=item[18], lidar2img_rts=item[19], map_location=item[20],
            )
