...
class RasterizedLocalMap(object):
    ...
    def convert_vec_to_mask(self, vectors):
        ...
        instance_masks = np.zeros((len(self.thickness), self.max_channel, self.canvas_size[0], self.canvas_size[1]), np.uint8)
        # pdb.set_trace()
    ...
    def line_geom_to_mask(self, layer_geom, idx, pbc_func, trans_type='index'):
        ...
        for line in layer_geom:
            ...
            if not new_line.is_empty:
                ...
                if new_line.geom_type == 'MultiLineString':
                    ...
                    new_coords = [(x, self.patch_size[0] - y) for x, y in new_line.coords]
                    # pts2 = np.array(self.patch_size[::-1])/2 + np.array(new_coords)
                    # pdb.set_trace()
                    ctr_points.append(pbc_func(np.array(new_coords)))
                    new_line = affinity.scale(new_line, xfact=scale_width, yfact=scale_height, origin=(0, 0))  # 线宽不变，只是坐标翻倍
        ...
        for i in range(len(self.thickness)):
            ...
            map_masks_ret.append(np.flip(map_masks[i][None], axis=1)[0])     # 因为line的坐标表示方式变了
            # pdb.set_trace()
